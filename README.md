# HiSL (Hindi Stopword List)
HiSL is a comprehensive Hindi stopword resource derived from multi-corpus statistical and linguistic analysis.

For more details, please refer to the published paper: 

## Resource Summary
* Total Stopwords: **1,311**
* Language: **Hindi**
* Script: **Devanagari**
* Construction Method:
  * Translation and validation of English and Hinglish stopwords
  * Aggregation of existing Hindi stopword resources
  * Statistical extraction using Zipf's law and knee-point detection
  * Manual linguistic validation

## Citation
If you use HiSL in your work, please cite:
```latex
@article{prasad2026hisl,
title={HiSL: A Comprehensive Hindi Stopword List Derived from Multi-Corpus Statistical and Linguistic Analysis},
author={Prasad, Uday Jaikishor and Mishra, Pramod Kumar},
journal={Proceedings of the National Academy of Sciences, India Section A: Physical Sciences},
year={2026}
}
```

## Repository Contents
* hisl.json — Final Hindi Stopword List (1,311 stopwords)
* README.md — Project documentation
* LICENSE — License information

## License
The HiSL stopword resource is licensed under the Creative Commons Attribution 4.0 International License (CC BY 4.0).
For the full license text, see: https://creativecommons.org/licenses/by/4.0/
